...
Setting settingHelper::parse_setting(string fname) {
    string line;
    ...
    if (myfile.is_open()) {
        ...
        while (getline(myfile,line)) {
            ...
            if ((line.substr(0,3).compare("max")) == 0) {
                ...
                size_t right_cb = 0;
                do { 
                    //Get barriers for each dimension of data in {...}, store as vector of vector
                    ...
                    if (right_cb-left_cb != 1){
                        do { 
                            //Get each barriers in the form of [x,y], store as vector of double
...
void settingHelper::create_setting()
{
    // int dimension = 0;
    double threshold = 0;
    int msp = 0, pd = 0;

...
public:
    ...
    /*！ 
     * \brief The main reporting mechanism in the library - when an anomaly is detected per process_input,
    ...
    /*!
     * \brief Note that AnomalyDetectionSystem does allocate memory, so the destructor is non-empty.
     */
    ...
    /*!
     * \brief The anomaly detection system processes the next provided data vector. Note that per temporal
    ...
    /*!
     * \brief After process_input has used its provided anomaly_detected_call_func to indicate an anamoly,
     ...
     * \param model_results a vector of doubles constains the results from each model
     * \param data, a vector of doubles, the input data
